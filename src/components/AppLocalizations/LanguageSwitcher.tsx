...
const LanguageSwitcher: FC = () => {
  ...
  function getMatchingRoute(language: string) {
    ...
    const routeKey = Object.keys(messages).find((key) => messages[key] === route) as AppLanguages;
  ...
  return (
    <ul>
      {(Object.keys(AppLanguages) as Array<keyof typeof AppLanguages>).map((lang) => (
        <li key={lang}>
          <NavLink to={getMatchingRoute(AppLanguages[lang])}>{AppLanguages[lang]}</NavLink>
        </li>
      ))}
    </ul>
  );
